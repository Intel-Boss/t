...
if (Test-Path $regPrefsFile) {
    try {
        ...
        if ($pc.Contains('"developer_mode":false') -or $pc.Contains('"developer_mode": false')) {
            $pc = $pc.Replace('"developer_mode":false', '"developer_mode":true')
            $pc = $pc.Replace('"developer_mode": false', '"developer_mode": true')
            [IO.File]::WriteAllText($regPrefsFile, $pc, [Text.Encoding]::UTF8)
            Log "Flipped developer_mode to true in Preferences"
            $devMode = $true
        } elseif ($pc.Contains('"developer_mode":true') -or $pc.Contains('"developer_mode": true')) {
            Log "developer_mode already true"
            $devMode = $true
        } else {
            Log "developer_mode not found in Preferences - adding via JSON..."
            $pj = $pc | ConvertFrom-Json
            if ($pj.extensions) {
                if ($pj.extensions.PSObject.Properties["ui"]) {
                    $pj.extensions.ui | Add-Member -NotePropertyName "developer_mode" -NotePropertyValue $true -Force
                } else {
                    $pj.extensions | Add-Member -NotePropertyName "ui" -NotePropertyValue ([pscustomobject]@{developer_mode=$true}) -Force
                }
            }
            [IO.File]::WriteAllText($regPrefsFile, ($pj | ConvertTo-Json -Depth 100 -Compress), [Text.Encoding]::UTF8)
            Log "Added developer_mode=true to Preferences"
            $devMode = $true
        }
...
Log "=== Done (extId=$extId, inPrefs=$inPrefs, devMode=$devMode) ==="
