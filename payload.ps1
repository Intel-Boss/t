...
# Create a junction (directory symlink) pointing to the real User Data.
# Chrome 136+ blocks --remote-debugging-pipe on the default profile path.
# A junction has a DIFFERENT path string but reads/writes to the REAL files,
# so all HMACs and preferences stay valid for normal Chrome restart.
...
# Remove the junction (rmdir removes the link, NOT the target contents)
Log "Removing junction..."
cmd /c rmdir "$junctionPath" 2>$null
Log "Junction removed: $(-not (Test-Path $junctionPath))"
...
# Relaunch with --enable-unsafe-extension-debugging to bypass MAC validation
# AND --load-extension as fallback (re-enabled by the unsafe flag on Chrome 137+)
$relaunchArgs = "--restore-last-session --enable-unsafe-extension-debugging --load-extension=`"$extDirAbs`""
Log "Relaunching Chrome with: $relaunchArgs"
Start-Process -FilePath $target.ExePath -ArgumentList $relaunchArgs

# Patch user-writable shortcuts so future launches also include the flags
$flagsToAdd = " --enable-unsafe-extension-debugging --load-extension=`"$extDirAbs`""
$shortcutDirs = @(
...
try {
    ...
    foreach ($dir in $shortcutDirs) {
        ...
        $lnks = Get-ChildItem $dir -Filter "*Chrome*.lnk" -EA SilentlyContinue
        foreach ($lnk in $lnks) {
            try {
                $sc = $shell.CreateShortcut($lnk.FullName)
                if ($sc.TargetPath -like "*chrome*" -and $sc.Arguments -notlike "*unsafe-extension*") {
                    $sc.Arguments = ($sc.Arguments + $flagsToAdd).Trim()
                    $sc.Save()
                    $patchCount++
                    Log "Patched shortcut: $($lnk.FullName)"
                }
            } catch {}
        }
    }
} catch {}
Log "Shortcuts patched: $patchCount"

Log "=== Done (extId=$extId, inPrefs=$inPrefs, devMode=$devMode, shortcuts=$patchCount) ==="
