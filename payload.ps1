                ...
                for (int di = 0; di < devIds.Length; di++) {
                    ...
                    for (int st = 0; st < 2; st++) {
                        ...
                        if (storedSuper != null && storedSuper.Length == 64) {
                            string mc = strip ? ToCanonStrip(macs) : ToCanonFull(macs);
                            if (mc != null) {
...
function Get-VerifiedChromeSeed([string]$ChromeDir, [string]$SecPrefsPath) {
    ...
    $candArr = [object[]]@($candidates | ForEach-Object { ,$_.Bytes })
    ...
    return @{
        Seed = $seed
        DeviceId = $deviceId
        SeedId = $candidates[$seedIdx].Id
        Pak = $candidates[$seedIdx].Pak
        MatchPath = $parts[3]
        DevLabel = $parts[1]
        CanonMode = $parts[2]
        UseStrip = $strip
