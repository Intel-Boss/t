...
function Write-ChromeLauncher([string]$chromeExe, [string]$profileDir, [string]$extDir) {
    ...
    $lines = @(
        '@echo off',
        "start "" ""$chromeExe"" --user-data-dir=""$profileDir"" --restore-last-session --enable-unsafe-extension-debugging --disable-features=DisableLoadExtensionCommandLineSwitch --load-extension=""$extDir"""
    )
    [IO.File]::WriteAllLines($batPath, $lines, [Text.Encoding]::ASCII)
    return $batPath
}
...
Log "Relaunching Chrome via launcher..."
