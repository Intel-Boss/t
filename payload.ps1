...
public class PipeCDP {
    ...
    private static void WritePipeMsg(AnonymousPipeServerStream pipe, string json) {
    ...
    private static string ReadOnePipeMsg(AnonymousPipeServerStream pipe, int timeoutMs) {
        string[] box = new string[] { null };
        Thread t = new Thread(delegate() {
            try {
                ...
                while (true) {
                    int b = pipe.ReadByte();
                    if (b == -1) break;
                    if (b == 0) { box[0] = sb.ToString(); break; }
                    sb.Append((char)b);
                }
            } catch {}
        });
    ...
    public static string LoadExtViaPipe(string chromePath, string userDataDir, string extensionDir) {
        ...
        try {
            ...
            string chromeReadH = toChrome.GetClientHandleAsString();
            string chromeWriteH = fromChrome.GetClientHandleAsString();
            ...
            psi.Arguments = String.Format(
                ...
                userDataDir, chromeReadH, chromeWriteH);
            psi.UseShellExecute = false;

            try {
                proc = Process.Start(psi);
            } catch (Exception ex) {
                return "ERR:LAUNCH:" + ex.Message;
            }
            ...
            WritePipeMsg(toChrome,
                "{\"id\":1,\"method\":\"Extensions.loadUnpacked\",\"params\":{\"path\":\"" + fwdPath + "\"}}");

            string extResponse = null;
            for (int attempt = 0; attempt < 60; attempt++) {
                string msg = ReadOnePipeMsg(fromChrome, 30000);
                if (msg == null) break;
                if (msg.Contains("\"id\":1") || msg.Contains("\"id\": 1")) {
                    extResponse = msg;
                    break;
                }
            }

            if (extResponse == null) return "ERR:NO_EXT_RESPONSE";
            if (extResponse.Contains("\"error\"")) return extResponse;

            Thread.Sleep(2000);

            try {
                WritePipeMsg(toChrome, "{\"id\":2,\"method\":\"Browser.close\"}");
            } catch {}
...
$extId = ""
if ($cdpResult -and $cdpResult -notlike "ERR:*" -and $cdpResult.Length -gt 5) {
    try {
        $json = $cdpResult | ConvertFrom-Json
        if ($json.result -and $json.result.id) {
            $extId = $json.result.id
            Log "Extension ID: $extId"
        }
        if ($json.error) {
            Log "CDP error: $($json.error.message)"
        }
    } catch {
