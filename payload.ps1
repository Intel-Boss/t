...
function Set-ExtensionPolicy([string]$policyPath, [string]$extId, [string]$updateUrl) {
    ...
    # Convert PS registry path to reg.exe format (HKCU:\Software\... -> HKCU\Software\...)
    $regPath = $policyPath -replace '^HKCU:\\', 'HKCU\' -replace ':', ''
    $regOut = reg add "$regPath" /v 1 /t REG_SZ /d "$value" /f 2>&1
    if ($LASTEXITCODE -ne 0) {
        # Fallback: try PowerShell method with explicit parent creation
        $parts = $policyPath -split '\\'
        $current = $parts[0]
        for ($i = 1; $i -lt $parts.Length; $i++) {
            $current = "$current\$($parts[$i])"
            if (-not (Test-Path $current)) {
                New-Item -Path $current -Force -EA SilentlyContinue | Out-Null
            }
        }
        Set-ItemProperty -Path $policyPath -Name "1" -Value $value -Force -EA SilentlyContinue
    }
