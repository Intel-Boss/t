...
public class PipeCDP {
    public static string LoadExtViaPipe(string chromePath, string userDataDir, string extensionDir) {
        ...
        try {
            ...
            string cmd = "{\"id\":1,\"method\":\"Extensions.loadUnpacked\",\"params\":{\"path\":\"" + fwdPath + "\"}}";
            byte[] cmdBytes = Encoding.UTF8.GetBytes(cmd);

            try {
                toChrome.Write(cmdBytes, 0, cmdBytes.Length);
                toChrome.WriteByte(0);
                toChrome.Flush();
            } catch (Exception ex) {
                return "ERR:PIPE_WRITE:" + ex.Message;
            }
